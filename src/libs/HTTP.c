...
size_t http_write_data(void *buffer, size_t size, size_t nmemb, void *userp) {
    ...
    data points to the start off the buffer -> size says how many bytes already exists 
...
It updates the provided city_node's city_data_t fields (temp, windspeed, rel_hum, cached_at)
with the values found in the JSON file.
*/
int http_load_cache(city_node_t *city_node, char *fp) {
  if (!city_node || !fp) {
    return STATUS_FAIL;
  ...
  if (jtemp && json_is_number(jtemp)) city_node->data->temp      = json_number_value(jtemp);
  if (jwind && json_is_number(jwind)) city_node->data->windspeed = json_number_value(jwind);
  if (jhum  && json_is_number(jhum))  city_node->data->rel_hum   = json_number_value(jhum);
