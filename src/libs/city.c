...
int city_boot(city_list_t *city_list) {
  ...
  for (unsigned i = 0; i < n; i++) {
    ...
    city_data_t *data = city_make_data(b->name, b->fp, b->lat, b->lon, INIT_VAL, INIT_VAL, INIT_VAL);
    if (!data) {
      continue;
    }
...
int city_read_cache(city_list_t *list) {
  ...
  while (dir.has_next) {
    ...
    if (!file.is_dir && strstr(file.name, ".json")) {
      ...
      if (!json_is_string(jname) || !json_is_string(jfp) || !json_is_number(jlat) || !json_is_number(jlon)) {
        json_decref(root);
        tinydir_next(&dir);
        continue;
      }
      ...
      */
      city_data_t *data = city_make_data((char *)json_string_value(jname), (char*)json_string_value(jfp), json_number_value(jlat), json_number_value(jlon), 
      ...
      */
      time_t cached = (jcached_at && json_is_integer(jcached_at)) ? (time_t)json_integer_value(jcached_at) : 0;
...
int city_save_cache(city_data_t *data) {
  ...
  /*Makes cache folder if it does not exist*/
...
city_data_t *city_make_data(char* city_name, char* fp, double lat, double lon,
    ...
    size_t fp_len = snprintf(NULL, 0, "./cities/%s_%.2f_%.2f.json", city_name, lat, lon) +1;
    ...
    snprintf(data->fp, fp_len, "./cities/%s_%.2f_%.2f.json", city_name, lat, lon);
